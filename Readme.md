...
A lightweight, internal IT support ticketing system for an organisation. Employees can raise support tickets with rich-text descriptions and screenshots, while the IT team can triage, assign, track, and resolve tickets through a Kanban-style dashboard — with automated email notifications on resolution.
...
| Database | SQLLite + Entity Framework Core 8 |
...
```
TicketingSystem/
├── TicketingSystem.Test/      			# Tests
├── TicketingSystem.Db/        			# EF Core DbContext, migrations, DB Model
├── TicketingSystem.Server/             # ASP.NET Core API (Minimal APIs)
│   ├── Endpoints/Authentication/       # Login, Register, Forgot Password
│   ├── Endpoints/Tickets               # Tickets Create, Delete, Fetch, Update
...
1. **Publish** the app:
   ```bash
   dotnet publish TicketingSystem.Server/TicketingSystem.Server.csproj -c Release -o C:\Publish\TPMS
   ```
